...
public class diff_match_patchTest : diff_match_patch {
 ...
 public void patch_make_apply_utf8Test() {
    List<Patch> patches;
    patches = this.patch_make("", "");
    assertEquals("patch_make: Null case.", "", this.patch_toText(patches));
    ...
    //1. make patch obj
    patches = this.patch_make(old, newText);
    ...
    var myPatch = this.patch_fromText(patchText);

	//4. apply patch to get new text    
    var results = this.patch_apply(patches, old);
   ...
   //resultStr = System.Web.HttpUtility.UrlDecode(resultStr, System.Text.Encoding.UTF8);
    ...
    assertEquals("patch_apply: Exact match.", newText, resultStr);
