...
namespace EmbedIDE
{
	...
	public class ProcessCreator
	{
		...
		/// <returns></returns>
		public static bool CreateProcess(int parentProcessId, string appName)
		{
			...
			try {
				...
				var lpApplicationName = appName; //Path.Combine(Environment.SystemDirectory, "notepad.exe");
				sInfoEx.StartupInfo.dwFlags =  STARTF_USESHOWWINDOW|STARTF_USEPOSITION | STARTF_USESIZE;
				//sInfoEx.StartupInfo.dwX = 1900;
				//sInfoEx.StartupInfo.dwX = 10;
				//sInfoEx.StartupInfo.dwXSize=90;
				//sInfoEx.StartupInfo.dwYSize = 90;
		...
		public static bool CreateProcessA(IntPtr parentHandle1, string appName)
		{
			const uint EXTENDED_STARTUPINFO_PRESENT = 0x00080000;
			const int PROC_THREAD_ATTRIBUTE_PARENT_PROCESS = 0x00020000;
			...
			try {
				if (parentHandle1 != IntPtr.Zero) {
					...
					var parentHandle = parentHandle1;//Process.GetProcessById(parentProcessId).Handle;
					// This value should persist until the attribute list is destroyed using the DeleteProcThreadAttributeList function
					lpValue = Marshal.AllocHGlobal(IntPtr.Size);
					Marshal.WriteIntPtr(lpValue, parentHandle);
